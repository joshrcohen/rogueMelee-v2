...
#define FIELD(name) word(c, &r->name)
    FIELD(version); FIELD(seed); FIELD(character); FIELD(act); FIELD(floor);
    ...
    FIELD(encounters); FIELD(gold_spent); FIELD(rerolls_used); FIELD(history_count);
...
int RogueRun_Deserialize(RogueRun* run, const unsigned char* data, unsigned size)
{
    ...
    codec(&c,&copy);
    if (!c.ok || c.pos != size || copy.version != 1 || copy.character >= 26 ||
